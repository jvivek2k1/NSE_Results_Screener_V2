...
// NOTE: The "alert-sql-blocking-high" alert lives in monitoring.bicep as a
// log-search alert over the app's SqlBlockedSessions telemetry. A metric alert
// on workers_percent was tried here but could never fire: blocked sessions sit
// idle waiting on locks, so workers_percent / cpu_percent stay flat (~0%) even
// during a 30+ session blocking storm. Blocking is only observable via a DMV
// query, which the app probes and emits as a custom metric.
