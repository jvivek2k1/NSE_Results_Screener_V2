...
_Last refreshed: 2026-06-29T11:16:10Z (from resource group `rg_jb-nse_290626_rg`)._

| Fact | Value |
|---|---|
| Subscription | `da4c3e1b-2de5-450b-99be-4c6400c2abdf` |
| Resource group | `rg_jb-nse_290626_rg` |
| Public URL (FQDN) | http://nse-290626114703.westus2.cloudapp.azure.com |
| Public IP | `20.230.233.156` |
| App Insights appId | `57de7221-d922-45e5-81b2-f25fb751d189` |
| AI deployment | `NSE_RESULTS_SCREENER_MODEL` on `nse-ai-290626114703` |

### Resources under management
| Component | Name |
|---|---|
| App Service (Node 20) | `app-290626114703` |
| App Service plan | `plan-290626114703` |
| Application Gateway (WAF v2) | `agw-290626114703` |
| Public IP / FQDN | `pip-290626114703 -> nse-290626114703.westus2.cloudapp.azure.com` |
| WAF policy | `waf-290626114703` |
| Virtual network | `vnet-290626114703` |
| Azure SQL server / db | `sql-290626114703 / JBDB` |
| Azure AI / Foundry | `nse-ai-290626114703` |
| Application Insights | `appi-290626114703` |
| Log Analytics | `log-290626114703` |
| Key Vault | `kv-290626114703` |
| Action group | `ag-290626114703` |
...
| `alert-sql-blocking-high`; sessions blocked, workers elevated | Scenario 9 (SQL blocking tree — **do not restart App Service**) |
...
**Blocking scenario (`alert-sql-blocking-high`) — strict rule:** an App Service restart is
**never** the remediation for a SQL blocking tree, and is **forbidden as an autonomous
action**. Even when operating autonomously you must obtain explicit user approval before
restarting / stopping / recycling App Service for any reason. For blocking, you must:
1. **Investigate in Azure SQL only** — run the blocking-tree DMV queries against `JBDB`
   (`sys.dm_exec_requests`, `sys.dm_exec_sessions`, blocking chain) to identify the head
   blocker, victims, and the offending statement; do not touch App Service.
2. **Explain** what is happening (head blocker SPID, lock held, sessions waiting).
3. **Ask the user** whether you may terminate (`KILL`) the head blocker session, then do it
   only after explicit approval. Verify the chain clears and the alert auto-mitigates.
