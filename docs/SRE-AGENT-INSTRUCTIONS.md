...
_Last refreshed: 2026-07-01T14:42:44Z (from resource group `rg_nse_1726_results_rg`)._

| Fact | Value |
|---|---|
| Subscription | `da4c3e1b-2de5-450b-99be-4c6400c2abdf` |
| Resource group | `rg_nse_1726_results_rg` |
| Public URL (FQDN) | http://nse-010726194448.westus2.cloudapp.azure.com |
| Public IP | `40.64.127.243` |
| App Insights appId | `fb9312e6-aa2c-4fd2-98ec-e955a5ffca0b` |
| AI deployment | `NSE_RESULTS_SCREENER_MODEL` on `nse-ai-010726194448` |

### Resources under management
| Component | Name |
|---|---|
| App Service (Node 20) | `app-010726194448` |
| App Service plan | `plan-010726194448` |
| Application Gateway (WAF v2) | `agw-010726194448` |
| Public IP / FQDN | `pip-010726194448 -> nse-010726194448.westus2.cloudapp.azure.com` |
| WAF policy | `waf-010726194448` |
| Virtual network | `vnet-010726194448` |
| Azure SQL server / db | `sql-010726194448 / JBDB` |
| Azure AI / Foundry | `nse-ai-010726194448` |
| Application Insights | `appi-010726194448` |
| Log Analytics | `log-010726194448` |
| Key Vault | `kv-010726194448` |
| Action group | `ag-010726194448` |
...
The SRE agent's **managed identity is provisioned as a contained database user in
the `JBDB` database**, so you can query Azure SQL **directly** (via its Entra token —
no SQL login/password) instead of relying only on App Insights telemetry. Use this
to inspect live DMVs, confirm root cause, and verify recovery first-hand.

Granted roles (least privilege — **read + DDL only, no DML/data changes**):
1. **`db_ddladmin`** — inspect and manage schema objects (indexes, statistics, etc.).
2. **`db_datareader`** — read all user tables and views.

Guidance:
- **Prefer direct queries** against `sql-010726194448 / JBDB` for live diagnosis
  (e.g. active sessions, blocking chains, `sys.dm_exec_requests`,
  `sys.dm_db_resource_stats` for CPU, index/stats health) — this is faster and more
  authoritative than waiting on telemetry ingestion.
- Authenticate with the agent's managed identity (Entra token); never use SQL auth.
- These roles grant **no INSERT/UPDATE/DELETE** on application data. Do **not**
  attempt data mutations. Any write/DDL remediation still follows the
  **Remediation policy** below (schema changes that alter app data or behaviour need
  human approval).
- Direct SQL access is a **read/diagnose** tool; it does not change the approval rules
  for destructive or infrastructure actions.
